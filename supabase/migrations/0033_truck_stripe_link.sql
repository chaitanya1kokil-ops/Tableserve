-- ============================================================================
-- Food trucks can drop in a simple Stripe payment link (Settings). If set, the
-- customer is redirected there after ordering — and, because a static link
-- can't confirm payment, the order is created 'new' (reaches the board right
-- away), exactly like a counter QR. Trucks WITHOUT a link keep the pay-first
-- Connect flow (order stays 'awaiting_payment' until /api/confirm-payment).
-- ============================================================================
...
begin
  ...
  select coalesce(tax_rate, 0), coalesce(business_type, 'restaurant'), stripe_link
  into v_rate, v_biz, v_link
  ...
  -- Trucks with a static payment link behave like a counter (order goes to the
  -- board now, pay via the link). Trucks without one use the Connect pay-first
  -- flow, so the order waits at 'awaiting_payment' until payment is confirmed.
  v_status := case
                when v_biz = 'food_truck' and coalesce(trim(v_link), '') = '' then 'awaiting_payment'
                else 'new'
              end;
